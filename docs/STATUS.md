...
- **Verified:** `npm ci`, `npm run build`, `npm run check`, `npm run typecheck`, `npm run docs:build` and `cargo test --workspace` on Node 24; `npm test` now terminates in one run — 217 tests, 217 pass, 0 skipped. The inherited hang and the three failures it hid are fixed ([review](review-a2.md)).
